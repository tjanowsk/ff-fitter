...
class Fitter:
    def __init__(self, jsonFilename):
        ...
        self.constraints = json_input["constraints"]
        self.formFactors = {}
        for dataFilename in json_input:
            if dataFilename == "constraints":
                continue
            with h5py.File(dataFilename, 'r') as f:
                qsqlist = list(f['qsqlist'][...])
                data = {key: f[key][...] for key in f}
                for key in json_input[dataFilename]:
                    mB = np.float(json_input[dataFilename][key]["mB"])
                    mpole = np.float(json_input[dataFilename][key]["m_pole"])
                    numParams = np.int(json_input[dataFilename][key]["num_pars"])
                    fitForm = json_input[dataFilename][key]["fit_form"]
                    lb, ub = getbounds(qsqlist, np.int(json_input[dataFilename][key]["lb"]), np.int(
                        json_input[dataFilename][key]["ub"]))
                    self.formFactors[key] = FormFactor(
                        qsqlist, data[key], mB, mpole, fitForm, numParams, lb, ub)
    ...
    def constraintsToSets(self):
        ...
        return [ [set(pat.findall(c)),c] for c in self.constraints if pat.findall(c) != []] +\
            [[set(pat2.findall(c)), c]
             for c in self.constraints if pat2.findall(c) != []]
    ...
    def evalConstraints(self, paramDict):
        ...
        for constraint in self.partialconstrs:
            constraint = re.sub("([0-9A-Za-z_-]+)\(([0-9A-Za-z_*.-]+)\)",
                                "self.formFactors[\"\g<1>\"].function(\g<2>, paramDict[\"\g<1>\"])", constraint)
            constraint = re.sub(
                "<([0-9A-Za-z_-]+)>", "self.formFactors[\"\g<1>\"].residue(paramDict[\"\g<1>\"])", constraint)
    ...
    def calculateResidue(self, parameters):
        ...
        result = np.concatenate(
            [self.formFactors[ff].calculateResidue(self.sampleNumber, paramDict[ff]) for ff in self.partialfflist] +
    ...
    def getResidues(self):
        residues = {name: [self.formFactors[name].residue(
            sample) for sample in self.fit[name]] for name in self.formFactors}
    ...
    def plot(self, fflist):
        ...
        for i, name in enumerate(fflist):
            ...
            yverr_below = np.std([ff.function(xv_below, sample)
                                  for sample in self.fit[name]], axis=0)
            yverr_above = np.std([ff.function(xv_above, sample)
                                  for sample in self.fit[name]], axis=0)
            ...
            plt.fill_between(xv_below, yvfit_below-yverr_below,
                             yvfit_below+yverr_below, color=color, alpha=0.3)
            plt.fill_between(xv_above, yvfit_above-yverr_above,
                             yvfit_above+yverr_above, color=color, alpha=0.3)
        plt.legend()
        plt.show()
